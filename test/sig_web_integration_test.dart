...
void main() {
  ...
  setUpAll(() async {
    // Configuration des mocks pour Firebase - version simplifiée
  ...
  group('UI Interaction Tests', () {
    ...
    testWidgets('Map type changes correctly', (WidgetTester tester) async {
      ...
      // Vérification que la fonction s'exécute sans erreur
      expect(find.byType(SigWeb), findsOneWidget);
    });
  });

  // Test simplifié sans mocks complexes
  testWidgets('Basic functionality test without complex mocks', (WidgetTester tester) async {
    ...
    // Vérifier que l'interface répond aux interactions
    expect(find.byType(SigWeb), findsOneWidget);
  });
}