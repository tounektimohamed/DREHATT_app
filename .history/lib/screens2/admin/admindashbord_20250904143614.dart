...
import 'package:DREHATT_app/screens2/jeojson/TestImagePage.dart';
...
class _AdminDashboardState extends State<AdminDashboard> {
                    ...
                    Expanded(
                      child: PageView(
                        ...
                        children: [
                          ...
                          // Page Géospatial
                          _buildCategoryPage("Géospatial", _geoItems, isDesktop, isTablet),
                          
                          // Page Gestion
                          _buildCategoryPage("Gestion", _managementItems, isDesktop, isTablet),
                          
                          // Page Demandes
                          _buildCategoryPage("Demandes", _requestItems, isDesktop, isTablet),
                        ],
                      ),
                    ),
                  ],
  ...
  Widget _buildNavItem(String title, IconData icon, int index) {
    return ListTile(
      ...
      title: Text(
        title,
        style: GoogleFonts.roboto(
          color: _currentIndex == index ? const Color(0xFF3A7FD5) : Colors.grey[700],
      ...
      child: Column(
        ...
        children: [
          ...
          if (isDesktop || isTablet)
            Row(
              children: [
                Expanded(child: _buildCategoryCard('Géospatial', Icons.map, const Color(0xFF48BB78), 1)),
                const SizedBox(width: 16),
                Expanded(child: _buildCategoryCard('Gestion', Icons.people, const Color(0xFF4299E1), 2)),
                const SizedBox(width: 16),
                Expanded(child: _buildCategoryCard('Demandes', Icons.request_page, const Color(0xFFED8936), 3)),
              ],
            )
          else
            Column(
              children: [
                _buildCategoryCard('Géospatial', Icons.map, const Color(0xFF48BB78), 1),
                const SizedBox(height: 12),
                _buildCategoryCard('Gestion', Icons.people, const Color(0xFF4299E1), 2),
                const SizedBox(height: 12),
                _buildCategoryCard('Demandes', Icons.request_page, const Color(0xFFED8936), 3),
              ],
            ),
        ],
      ),
  ...
  Widget _buildCategoryPage(String title, List<Map<String, dynamic>> items, bool isDesktop, bool isTablet) {
    int crossAxisCount;
    if (isDesktop) {
      crossAxisCount = 4;
    } else if (isTablet) {
  ...
  Widget _buildCategoryCard(String title, IconData icon, Color color, int pageIndex) {
    return Card(
      elevation: 2,
  ...
  Widget _buildDashboardItem(String title, String imagePath, Color color, VoidCallback onTap) {
    return Card(
      elevation: 2,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
