...
class _SatelliteMonitorPageState extends State<SatelliteMonitorPage> {
  ...
  bool modelLoaded = false; // ← AJOUTÉ: Variable pour suivre l'état du modèle
  ...
  void initState() {
    super.initState();
    _checkServerStatus();
    _checkModelStatus(); // ← AJOUTÉ: Vérifier le statut du modèle
  }

  // ← AJOUTÉ: Méthode pour vérifier le statut du modèle IA
  Future<void> _checkModelStatus() async {
    try {
      final response = await http.get(Uri.parse("$baseUrl/model_status"))
      ...
      print("Erreur lors de la vérification du modèle: $e");
    }
  }

  Future<void> aiCompare() async {
    setState(() {
      loading = true;
      errorMessage = null;
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data["status"] == "success") {
          ...
          setState(() {
            aiChangePercentage = data['change_percentage']?.toDouble();
            if (data['result_image'] != null) {
              aiAnnotatedImageUrl =
                  "$baseUrl/images/${data['result_image']}?t=$timestamp";
            }
            loading = false;
          });
          ...
          showDialog(
            ...
            builder: (context) => AlertDialog(
              ...
              content: Text(
                  "Changement détecté: ${aiChangePercentage?.toStringAsFixed(2)}%"),
              actions: [
                TextButton(
  ...
  Future<void> _checkServerStatus() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _getChanges();
        _checkModelStatus(); // ← AJOUTÉ: Vérifier le modèle après connexion
      }
    } catch (e) {
      _tryNextUrl();
    }
      ...
      if (response.statusCode == 200) {
        ...
        if (data["status"] == "success") {
          ...
          setState(() {
            ...
            if (data['ai_annotated_path'] != null) {
              aiAnnotatedImageUrl =
                  "$baseUrl/images/${data['ai_annotated_path']}?t=$timestamp";
            }
            if (data['ai_change_percentage'] != null) {
              aiChangePercentage = data['ai_change_percentage'].toDouble();
            }
            loading = false;
          });

          if (data['change_ratio'] != null || data['ai_change_percentage'] != null) {
            final changeRatio = data['change_ratio']?.toDouble() ?? 0.0;
            final aiChange = data['ai_change_percentage']?.toDouble() ?? 0.0;
            
            showDialog(
              context: context,
              builder: (context) => AlertDialog(
                title: const Text("Résultats de la comparaison"),
                content: Column(
                  mainAxisSize: MainAxisSize.min,
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Méthode traditionnelle: ${changeRatio.toStringAsFixed(2)}%"),
                    const SizedBox(height: 10),
                    Text("Méthode IA: ${aiChange.toStringAsFixed(2)}%"),
                  ],
                ),
                actions: [
                  TextButton(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          aiAnnotatedImageUrl = null;
          aiChangePercentage = null;
        });
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            // Status du serveur et du modèle
            Column(
              children: [
                // Status du serveur
                Card(
                ...
                // Status du modèle IA
                Card(
                  color: modelLoaded ? Colors.green[100] : Colors.orange[100],
                  child: Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: Row(
                      children: [
                        Icon(
                          modelLoaded ? Icons.check_circle : Icons.warning,
                          color: modelLoaded ? Colors.green : Colors.orange,
                        ),
                        const SizedBox(width: 8),
                        Text(
                          "Modèle IA: ${modelLoaded ? 'Chargé' : 'Non disponible'}",
                          style: TextStyle(
                            fontWeight: FontWeight.bold,
                            color: modelLoaded ? Colors.green : Colors.orange,
                          ),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        ElevatedButton.icon(
                          onPressed: modelLoaded ? aiCompare : null,
                          icon: const Icon(Icons.psychology),
                          label: const Text("Analyse IA"),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: modelLoaded ? Colors.purple : Colors.grey,
            ...
            if (originalImageUrl != null || aiAnnotatedImageUrl != null)
              Expanded(
                child: ListView(
                  children: [
                    ...
                    if (aiAnnotatedImageUrl != null)
                      _buildImageCard(
                          "Résultat IA", aiAnnotatedImageUrl!),
                  ],
                ),
              )
            else if (!loading &&
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final aiChange = change['ai_change_percentage']?.toStringAsFixed(2) ?? 'N/A';
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    title: Text("Traditionnel: $changeRatio% | IA: $aiChange%"),
                    subtitle: Text(
                    ...
                    trailing: IconButton(
                      ...
                      onPressed: () {
                        ...
                        setState(() {
                          ...
                          if (change['ai_annotated_path'] != null) {
                            aiAnnotatedImageUrl =
                                "$baseUrl/images/${change['ai_annotated_path']}?t=$timestamp";
                          }
                          showChanges = false;
                        });
                      },
