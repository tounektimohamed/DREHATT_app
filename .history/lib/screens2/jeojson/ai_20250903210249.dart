...
class _GeoSurveillanceScreenState extends State<GeoSurveillanceScreen> {
      ...
      for (int y = 0; y < image1.height; y++) {
        for (int x = 0; x < image1.width; x++) {
          if (mask[y][x] > 0) {
            ...
            final rDiff = (img.getRed(pixel1) - img.getRed(pixel2)).abs();
            final gDiff = (img.getGreen(pixel1) - img.getGreen(pixel2)).abs();
            final bDiff = (img.getBlue(pixel1) - img.getBlue(pixel2)).abs();
  ...
  List<List<int>> _createPolygonMask(int width, int height, List<ui.Offset> polygon) {
    ...
    final canvasUi = ui.Canvas(canvas);
    final paint = ui.Paint()..color = const ui.Color(0xFFFFFFFF);
    
    canvasUi.drawPath(path, paint);
    
    final picture = canvas.endRecording();
    final image = picture.toImage(width, height);
    
    // Note: Cette approche est simplifiée. Une implémentation complète nécessiterait
    // un calcul plus sophistiqué pour déterminer quels pixels sont à l'intérieur du polygone.
    
    // Pour cette démo, nous utilisons un rectangle simple
