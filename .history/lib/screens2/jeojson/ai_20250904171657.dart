...
// Remplacer la fonction captureImage par:
    ...
    if (response.statusCode == 200) {
      ...
      if (data["status"] == "success") {
        ...
        setState(() {
          ...
          // Afficher le taux de différence
          if (data['change_ratio'] != null) {
            ...
            showDialog(
              ...
              builder: (context) => AlertDialog(
                ...
                content: Text("Taux de différence: ${changeRatio.toStringAsFixed(2)}%"),
                actions: [
                  TextButton(
              ...
              children: [
                ElevatedButton.icon(
                  onPressed: () async {
                    try {
                      final response = await http
                          .get(Uri.parse("$baseUrl/reset_comparison"));
                      if (response.statusCode == 200) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                              content: Text("Comparaison réinitialisée")),
                        );
                      }
                    } catch (e) {
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        if (changeCenter != null)
                          Text(
                              "Position: ${changeCenter[0]?.toStringAsFixed(4)}, ${changeCenter[1]?.toStringAsFixed(4)}"),
                      ],
                    ),
                    trailing: IconButton(
                      ...
                      onPressed: () {
                        ...
                        setState(() {
                          annotatedImageUrl =
                              "$baseUrl/images/${imagePath.replaceAll('.png', '_with_polygon.png')}?t=$timestamp";
                          originalImageUrl =
                              "$baseUrl/images/$imagePath?t=$timestamp";
                          showChanges = false;
                        });
                      },
                    ),
