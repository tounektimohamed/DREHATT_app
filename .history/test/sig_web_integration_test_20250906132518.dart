...
void main() {
  ...
  group('GeoJSON Processing Tests', () {
    testWidgets('Polygons are correctly parsed from GeoJSON', (WidgetTester tester) async {
      ...
      // Appeler la méthode de traitement GeoJSON
      await SigWebTestHelper.simulateGeoJsonLoading(sigWeb, testGeoJson);
      await tester.pumpAndSettle();

      // Pour vérifier le résultat, nous devrions normalement accéder à l'état interne
      // Mais comme c'est privé, nous vérifions plutôt le comportement visible
      // Dans un vrai test d'intégration, vous vérifieriez que la carte affiche les bons éléments
    });
    ...
    testWidgets('Polylines are correctly parsed from GeoJSON', (WidgetTester tester) async {
      ...
      // Vérification similaire au test précédent
    });
  });
